...
export async function createSwapTransaction(
  ...
  try {
    ...
    const txHash = await writeContract(web3Config, {
      address: PANGOLIN_ROUTER_ADDRESS,
...
export const getAmountOut = async (
  ...
  try {
    ...
    const pairContractAddress = await readContract(web3Config, {
      address: PANGOLIN_FACTORY_ADDRESS as `0x${string}`,
      abi: PANGOLIN_FACTORY_ABI,
    ...
    const quoteResult = await readContract(web3Config, {
      address: PANGOLIN_ROUTER_ADDRESS as `0x${string}`,
      abi: PANGOLIN_ROUTER_ABI,
